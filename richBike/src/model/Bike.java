...
public class Bike {
     ...
     * @param preis     Preis
     */
    public Bike(String rahmennr, String markeType, String text, BigDecimal preis, String farbe, Date date) {
        setRahmennr(rahmennr);
        setMarkeType(markeType);
        setText(text);
        setPreis(preis);
        setFarbe(Farbe.valueOf(farbe));
    ...
    private void fillAndKill() throws BikeException {
        ...
        if (date.get() == null) {
            throw new BikeException("Datum muss angegeben werden!");
        }
    }
    ...
    public void save() throws BikeException, SQLException, IOException {
        ...
        try {
            ...
            pstmt.setDate(6, Date.valueOf(getDate()));
            pstmt.execute();
