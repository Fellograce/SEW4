...
public class Bike {
    ...
    private NullableBigDecimalStringConverter decimalStringConverter = new NullableBigDecimalStringConverter(NumberFormat.getInstance());
    ...
    public static Bike select(String rahmennr) throws SQLException {
        Database.getInstance().getSelectStatement().setString(1, rahmennr);
        ResultSet resultSet = Database.getInstance().getSelectStatement().executeQuery();
     ...
     * @throws IOException
     */
    public void save() throws BikeException, SQLException {
        fillAndKill();

        try {
            // Insert versuchen
            Database.getInstance().getInsertStatement().setString(1, getRahmennr());
            Database.getInstance().getInsertStatement().setString(2, getMarkeType());
            Database.getInstance().getInsertStatement().setString(3, getText());
            Database.getInstance().getInsertStatement().setBigDecimal(4, getPreis());
            Database.getInstance().getInsertStatement().executeUpdate();
            ...
            if (e.getSQLState().equals("23505")) {
                // Update versuchen
                Database.getInstance().getUpdateStatement().setString(1, getMarkeType());
                Database.getInstance().getUpdateStatement().setString(2, getText());
                Database.getInstance().getUpdateStatement().setBigDecimal(3, getPreis());
                Database.getInstance().getUpdateStatement().setString(4, getRahmennr());
                Database.getInstance().getUpdateStatement().executeUpdate();
            } else {
                throw e;
            }
        }
