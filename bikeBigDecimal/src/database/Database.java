...
public class Database {
    ...
    private Database() {
        ...
        try {
            ...
            insertStatement = connection.prepareStatement("INSERT INTO Bike (rahmennr, markeType, text, preis) values (?,?,?, ?)");
            updateStatement = connection.prepareStatement("UPDATE Bike SET markeType = ?, text = ?, preis = ? WHERE rahmennr = ?");
            selectStatement = connection.prepareStatement("SELECT rahmennr, markeType, text, preis FROM Bike WHERE rahmennr = ?");
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(2);
        }
